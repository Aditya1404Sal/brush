...
///
/// On `wasm32` clank runs each non-last pipeline stage to completion *inline* and in order (see
...
mod mem_pipe {
    ...
    impl std::io::Write for MemPipeWriter {
        fn write(&mut self, data: &[u8]) -> std::io::Result<usize> {
            self.0.lock().unwrap().buf.extend(data.iter().copied());
            Ok(data.len())
        }
        fn flush(&mut self) -> std::io::Result<()> {
            Ok(())
    ...
    impl std::io::Write for MemPipeReader {
        fn write(&mut self, _buf: &[u8]) -> std::io::Result<usize> {
            Err(std::io::Error::other(super::error::ErrorKind::OpenFileNotWritable("pipe reader")))
        }
        fn flush(&mut self) -> std::io::Result<()> {
            Ok(())
        }
