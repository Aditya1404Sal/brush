...
/// Opens a null-like file: there are no device nodes on this platform, so `/dev/null` is emulated
/// with a truncate-on-open backing file — every open sees an empty file (reads yield EOF) and
/// bytes written are discarded by the next open. Not byte-for-byte `/dev/null` (bytes exist until
/// the next open), but redirect semantics (`> /dev/null`, `2>/dev/null`, `< /dev/null`) hold.
